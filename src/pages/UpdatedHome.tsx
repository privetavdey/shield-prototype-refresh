...
export default function UpdatedHome() {
  ...
  useEffect(() => {
    if (isRiveVisible && rive) {
      console.log('Playing Rive animation from useEffect');
      ...
      const hideTimeout = setTimeout(() => {
        ...
        setTimeout(() => {
          rive.stop();
          rive.reset({ stateMachines: 'State Machine 1' });
        }, LOADER_ANIMATION.DURATION * 1000);
  ...
  const handleRefreshClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('Refresh button clicked, isRiveVisible:', isRiveVisible, 'rive:', rive);
    
    // Reset state machine and show with configured delay
    if (rive) {
      rive.stop();
      rive.reset({ stateMachines: 'State Machine 1' });
    }
    ...
    setTimeout(() => {
      ...
      if (rive) {
        console.log('Playing Rive animation');
        rive.play('State Machine 1');
      }
    }, LOADER_ANIMATION.RESET_DELAY);
  };
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: SPACING.xs, alignItems: 'center' }}>
          {/* Eye button - does not trigger Rive animation */}
          <div
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          {/* Refresh Button - triggers Rive animation */}
          <div
